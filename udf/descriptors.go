...
originally ported from https://github.com/mogaika/udf
...
func r_timestamp(b []byte) time.Time {
	...
	return t.Add(time.Duration(b[6])*time.Hour + time.Duration(b[7])*time.Minute + time.Duration(b[8])*time.Second)
}
